...
def _get_phase_and_num_tokens(input_tensor: Optional[torch.Tensor]) -> tuple[str, int]:
    phase = "unknown"
    num_tokens = int(input_tensor.shape[0]) if input_tensor is not None and input_tensor.ndim > 0 else -1
    ...
    attn_state_name = getattr(attn_state, "name", str(attn_state)) if attn_state is not None else ""
    if attn_state_name in {"DecodeOnly", "SpecDecoding"}:
        phase = "decode"
    elif attn_state_name in {"PrefillNoCache", "PrefillCacheHit", "ChunkedPrefill"}:
        phase = "prefill"
...
def _record_entry(entry: dict[str, Any]) -> None:
    ...
    output_file = output_dir / f"layer_profile_rank{_get_rank()}_pid{os.getpid()}.jsonl"
    with _WRITE_LOCK:
